...
public class CUBIC {
    ...
    final static int SRC2SINK_QUEUE_SIZE = 5 ;
    final static int SINK2SRC_QUEUE_SIZE = 5 ;
    ...
    public static void main(String[] args) {
        for (int u = 0; u < 1; u++) {
            glb_source_id =0 ; glb_time=0 ;glb_packet_id=0 ;glb_source_id =0 ;curr_que_size_src2sink=0;
            global_Queue.clear();    
            ...
            while (glb_time <= time_of_simulation) {
                // System.out.println("global time: " + glb_time);
                        ...
                        case RECIEVED_AT_SOURCE:
                            ...
                            if(temp == 2) {
                                if(tcp.window_size > 1) {
                                    tcp.w_max = tcp.window_size; // update the last window size before reduction.
                                    tcp.last_congestion_time = (int)glb_time;
                                    tcp.window_size = tcp.window_size/2;
                                ...
                                if( tcp.rcvWindowAllAcked() ) {
                                    ...
                                    tcp.update_window_size(glb_time);
                                    curr_window_size.add(tcp.window_size);          
    ...
    static class TCP {
        ...
        public TCP(int window_size, double timeout) {
            ...
            this.w_max = 1;
            this.beta = 100;
            this.last_congestion_time = 0;
        }

        public void update_window_size(double t) {
            t -= this.last_congestion_time;
            K = Math.cbrt(this.w_max * this.beta / this.c);
            this.window_size = c*(int)Math.pow(t-K, 3) + w_max;
            // System.out.println("The new window_size is: " + this.window_size);
        }
        ...
        public void sendPackets(ArrayList<Packet> produced_packet, double timestamp) {
            ...
            if(packet_window_start+window_size<=produced_packet.size()) {
                for(int i=packet_window_start;i<packet_window_start+window_size;++i) {
                    ...
                    global_Queue.add(new Event(SENT_BY_SOURCE,produced_packet.get(i),timestamp));
                    global_Queue.add(new Event(SENT_BY_SOURCE,produced_packet.get(i),timestamp+timeout));
                } 
            }
