...
{
public:
	double producing_delay; int id, queue_size;
	static int counter;
...
int packet_size=500, queue_max_size=1000;
...
tuple<int,int,int> simulate(vector<int> lambda) {
	...
	int done_counter[3]={0}, dropped_count[3]={0, 0, 0}, total_drop=0;
	...
	for (int i = 0; i < 3; ++i)
	{
		...
		event starter(1.0, i, 0, sources[i].id);
		pq.push(starter);
	}
	network_switch net_switch;

	while(done + total_drop < limit) {
		...
		switch(current_event.event_type) {
			case 0 : {
				...
				*/
				if(sources[current_event.source_id].queue_size >= queue_max_size) {
					dropped_count[current_event.source_id]++;
					total_drop++;
				} else {
					sources[current_event.source_id].queue_size++;
			...
			case 1: {
				if(source_to_switch[current_event.source_id].busytill > current_event.timestamp) {
					...
					*/
					if(sources[current_event.source_id].queue_size > 0) {
						sources[current_event.source_id].queue_size--; // decrement the queue of the source.
					}

					if(net_switch.queue_size >= queue_max_size) {
						dropped_count[current_event.source_id]++;
						total_drop++;
						break;
					}
					net_switch.queue_size++;
	...
	return {dropped_count[0], dropped_count[1], dropped_count[2]};
}

int main() {
	vector<int> lambda(3);
	printf("lambda,DroppedA,DroppedB,DroppedC\n");
	for(int i = 10; i <= 50; i++) {
		lambda[0]=i-1; lambda[1]=i; lambda[2]=i+1;
		tuple<int,int,int> data = simulate(lambda);
